...
void gpu_execute_step(tick ticks) {
   ...
   if (gpu_timer >= 456) {
      ...
      if (gpu_scanline ==
          144) // The screen is 144 lines tall, we've now completed 0-143
      {
   ...
   if (gpu_mode ==
       gpu_mode_hblank) // Happens every time one complete scanline is drawn
   {

   } else if (gpu_mode ==
              gpu_mode_vblank) // Happens every time the full screen is drawn
   {
      if (!gpu_draw_frame) {
   ...
   } else if (gpu_mode == gpu_mode_scan_OAM) // These two are split into 2 parts
   // because the status can be checked
   // via a register
   {
   ...
   mem_direct_write(
      0xFF44,
      gpu_scanline &
         0xFF); // Writing here normally resets it, so we set it directly
...
void gpu_do_scanline() {
   ...
   // i Divide by 8 because each tile is 8x8 px and we want to get which tile
   // we're on
   word bg_tile_map_offset  = (((gpu_scanline + scrollY) & 0xFF) >> 3);
   word win_tile_map_offset = ((gpu_window_scanline & 0xFF) >> 3);

   bg_tile_map_offset *= 32;
   win_tile_map_offset *= 32;
   ...
   int bg_current_tile = mem_direct_read(bg_tile_map_location +
                                         bg_tile_map_offset + bg_x_tile_offset);
   if (which_tile_data && bg_current_tile < 128)
      bg_current_tile += 256;

   int win_current_tile = mem_direct_read(
   ...
   for (int i = 0; i < 160; i++) {
      ...
      if (lo & mask)
         col += 1;
      outcol = gpu_pick_color(
         col, mem_direct_read(0xFF47)); // This is correct if there is no window

      if (using_window == false) {
         gpu_vram[output_addr++] = outcol;
         gpu_vram[output_addr++] = outcol;
         gpu_vram[output_addr++] = outcol;
         gpu_vram[output_addr++] =
            0xFF; // We don't want to change the alpha channel
      ...
      if (y <= gpu_scanline && y + (big_sprites ? 16 : 8) > gpu_scanline) {
         ...
         if (!yflip)
            ...
            stempaddr =
               0x8000 + temptile * 16 +
               ((big_sprites ? 32 : 14) -
                ((gpu_scanline - y) & y_mask) * 2); // TODO: There's an off by 1
         // error here with big sprites,
         ...
         for (int sx = 0; sx < 8; sx++) {
            ...
            if (slo & smask)
               scol++;
            byte color =
               gpu_pick_color(scol, mem_direct_read(0xFF48 + pallete));
            if ((x + sx) < 160 &&
                scol) // Doesn't check priority with background. Fix later.
            {
               outcol          = color;
