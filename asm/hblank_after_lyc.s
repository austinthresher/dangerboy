...
-  ld (hl+), a
   ...
   ; Starting with LYC = 1, wait for LYC interrupt, change STAT to HBLANK, and
   ; count the number of HBLANK interrupts until VBLANK. Increment LYC and
   ; repeat until LYC == 90.
   ld b, $FF 
...
print_loop:
   ...
   call print_newline
   print_string_literal "IF ORDERED>=2,HBLANK"
   call print_newline
   print_string_literal "FIRES AFTER ALL LYC"
   ret
